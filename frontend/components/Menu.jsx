                        ...
                        {showCartMenu && (
                            <ul className='bg-white absolute top-6 left-0 min-w-[250px] px-1 text-black shadow-lg'>
                                {subMenuData.map((submenu)=>{
                                    return(
                                        <Link key={submenu.id} href='/' onClick={()=>setShowCartMenu(false)}>
                                            <li className='h-12 flex justify-between items-center px-3 py-1 hover:bg-black/[0.05] rounded-md'>
                                                {submenu.name}
                                            <span className='opacity-50 text-sm'>71</span>
                                            </li>
                                        </Link>
                                    )
                                })}
